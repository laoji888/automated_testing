...
currentime = time.strftime('%Y-%m-%d %H;%M;%S', time.localtime())  # 获取当前时间
case_path = "./case"  # 测试用例路径
file_path = './report_api'  # 测试报告路径
...
def email(file_path, to_email):  # 以附件方式把测试报告发送给指定的人
    ...
    name = os.listdir('./report_api')  # 以list的方式返回report下的报告
...
def my_tese():
    discover = unittest.defaultTestLoader.discover(case_path, pattern="api*.py")
    return discover


if __name__ == "__main__":
    report_path = "./report_api/" + "API_" + currentime + ".html"  # 报告保存路径
